...
class McWilliams2DDataset(Dataset):
    def __init__(self, file_path, 
                 data_res=(128, 128),  # spatial resolution for data
                 pde_res=(128, 128),   # spatial resolution for PDE loss
                 raw_res=(128, 128, 30),  # raw data resolution (H, W, T)
                 n_samples=None,       # number of samples to load
                 total_samples=None,   # total samples in dataset
                 idx=0,                # starting index
                 offset=0,             # offset for sampling
                 t_duration=1.0):      # duration of time sequence
        super().__init__()
        self.file_path = file_path
        self.data_res = data_res    # (H, W) for data
        self.pde_res = pde_res      # (H, W) for PDE loss
        self.raw_res = raw_res      # (H, W, T) raw data resolution
        self.t_duration = t_duration
        self.n_samples = n_samples
        self.offset = offset
        self.idx = idx
        ...
        if t_duration == 1.0:
            self.T = self.raw_res[2]  # Use all time steps
        else:
            self.T = int(self.raw_res[2] * t_duration) + 1
        ...
        self.data_s_step = pde_res[0] // data_res[0]
        self.data_t_step = (self.raw_res[2] - 1) // (self.T - 1)

    def load(self):
        """Load the McWilliams2D data from .pt file"""
        print(f"Loading McWilliams2D data from {self.file_path}")
        
        # Load the data
        raw_data = torch.load(self.file_path, map_location='cpu')  # Shape: (1000, 128, 128, 30, 2)
        vorticity = raw_data['vorticity'][0].squeeze().numpy()
        vorticity = np.transpose(vorticity, (1, 2, 0)).expand_dims(-1)  # (128, 128, 30, 1)
        stream = raw_data['stream'][0].squeeze().numpy()
        stream = np.transpose(stream, (1, 2, 0)).expand_dims(-1)  # (128, 128, 30, 1)
        ...
        sub_t = (self.raw_res[2] - 1) // (self.T - 1) if self.T < self.raw_res[2] else 1
        
        # Subsample data: (N, H, W, T, 2) -> (N, H_sub, W_sub, T_sub, 2)
        data = raw_data[self.offset:self.offset + self.n_samples, 
                       ::sub_h, ::sub_w, ::sub_t, :]
        
        # For PDE loss resolution (a_data), use different subsampling
        a_sub_h = self.raw_res[0] // self.pde_res[0]
        a_sub_w = self.raw_res[1] // self.pde_res[1]
        
        # Create initial condition data for PDE loss (first time step only)
        a_data = raw_data[self.offset:self.offset + self.n_samples, 
                         ::a_sub_h, ::a_sub_w, 0:1, :]  # Only first time step
        
        # Convert to torch tensors
        data = torch.from_numpy(data).to(torch.float32)  # (N, H, W, T, 2)
        a_data = torch.from_numpy(a_data).to(torch.float32)  # (N, H, W, 1, 2)
        
        # Reshape for compatibility with existing framework
        # data: (N, H, W, T, 2) -> (N, T, H, W, 2)
        data = data.permute(0, 3, 1, 2, 4)
        # a_data: (N, H, W, 1, 2) -> (N, 1, H, W, 2)
        a_data = a_data.permute(0, 3, 1, 2, 4)
        ...
        print(f"Initial condition shape: {self.a_data.shape}")
        print(f"Grid shape: {self.grid.shape}")

    def __getitem__(self, idx):
        """Get a single sample"""
        # Get the full trajectory
        trajectory = self.data[idx]  # (T, H, W, 2)
        
        # Get initial condition and repeat for all time steps
        ic = self.a_data[idx]  # (1, H, W, 2)
        ic_repeated = ic.repeat(self.T, 1, 1, 1)  # (T, H, W, 2)
        
        # Concatenate grid coordinates with initial condition
        # grid: (H, W, T, 3), ic_repeated: (T, H, W, 2) -> need to permute
        ic_for_concat = ic_repeated.permute(1, 2, 0, 3)  # (H, W, T, 2)
        
        # Concatenate grid coordinates with initial condition
        a_data = torch.cat((self.grid, ic_for_concat), dim=-1)  # (H, W, T, 5)
        
        return trajectory, a_data
